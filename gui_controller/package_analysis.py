...
class PackageController():
    ...
    def uninstall_one(self,sno,package_name):
        uninstall_result = os.popen('adb -s %s uninstall %s'%(sno,package_name)).read()
    ...
    '''
    def install_one_device2(self,sno,apk_name,apk_package_name):
        had_package = os.popen('adb -s %s shell pm list packages |findstr "youku.phone"'%sno).read()
        if re.search(r'com\.youku\.phone',had_package):
            self.uninstall_one(sno,apk_package_name)
        install_result = os.popen('adb -s %s install %s'%(sno,apk_name)).read()
        if re.findall(r'Success',install_result):
            print '>>>%s adb install %s [SUCCESS]' %(sno,apk_name)
        else:
            print '>>>%s install %s [FALSE]'%(sno,apk_name)
    def install_one_device(self,sno,apk_name,apk_package_name):
        had_package = os.popen('adb -s %s shell pm list packages |findstr "%s"'%(sno,apk_package_name)).read()
    ...
    def cover_install(self,sno,apk_name,apk_package_name):
        nstall_result = os.popen('adb -s %s install %s'%(sno,apk_name)).read()
        boolean = self.is_has_package(sno,apk_package_name)
        if re.findall(r'Success',install_result) or boolean:
            print '>>>[%s] adb install %s [SUCCESS]' %(sno,os.path.basename(apk_name))
        else:
    ...
    def is_has_package(self,sno,package_name):
        had_package = os.popen('adb -s %s shell pm list packages |findstr "%s"'%(sno,package_name)).read()
    ...
    def clear_app_data(self,sno,package_name):
        ...
        if b:
            res = os.popen("adb -s %s shell pm clear %s"%(sno,package_name)).read()
